    ...
    private static bool TryParsePath(Expression expression, out string path)
    {
      ...
      else if (methodCallExpression != null)
      {
        if (methodCallExpression.Method.Name == "Select" && methodCallExpression.Arguments.Count == 2)
        {
          string path1;
          string path2;
          if (!ExpressionExtensions.TryParsePath(methodCallExpression.Arguments[0], out path1) || (path1 == null || !(methodCallExpression.Arguments[1] is LambdaExpression lambdaExpression)) || (!ExpressionExtensions.TryParsePath(lambdaExpression.Body, out path2) || path2 == null))
            return false;
          path = path1 + "." + path2;
          return true;
        }
